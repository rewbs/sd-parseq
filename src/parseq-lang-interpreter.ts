...
import {ParseqAstNode, InvocationContext} from './parseq-lang/parseq-lang-ast';
import { toFixedNumber, toFixedCeil, toFixedFloor } from './utils';


export function defaultInterpolation(definedFrames : number[], definedValues : number[], frame : number) {
  return linear_interpolation(definedFrames, definedValues, frame);
}

function linear_interpolation(definedFrames : number[], definedValues : number[], frame : number) {
  return linear(frame, definedFrames, definedValues)[0];
}

function cubic_spline_interpolation(definedFrames : number[], definedValues : number[], frame : number) {
...
function poly_interpolation(definedFrames : number[], definedValues : number[], frame : number) {
  return polynomial(frame, definedFrames, definedValues)[0];
}

 // eslint-disable-next-line  no-unused-vars
function step_interpolation(definedFrames : number[], definedValues : number[], frame : number) {
...
export function interpret(ast : ParseqAstNode, ctx : InvocationContext) : number | string {

  console.log(JSON.stringify(ast.debug(), null, 2));
  //console.log(ast);
  return ast.invoke(ctx);
